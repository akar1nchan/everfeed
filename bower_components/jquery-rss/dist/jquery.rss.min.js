...
c.prototype.load=function(a){var b="http"+(this.options.ssl?"s":"")+"://ajax.lug.ustc.edu.cn/ajax/services/feed/load?v=1.0&output="+this.options.outputMode+"&callback=?&q="+encodeURIComponent(this.url);null!=this.options.limit&&(b+="&num="+this.options.limit);null!=this.options.key&&(b+="&key="+this.options.key);e.getJSON(b,a)};c.prototype.render=function(){var a=this;this.load(function(b){try{a.entries=b.responseData.feed.entries}catch(d){return a.entries=[],a.options.error.call(a)}b=a.generateHTMLForEntries();
